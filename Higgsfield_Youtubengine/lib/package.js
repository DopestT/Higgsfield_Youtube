...
/**
 * Shared production-package engine.
 *
 * Both `new-video` (generic idea) and `trend-video` (RSS trend) build a `ctx`
 * object and hand it to writePackage(), which renders the 10 deliverable files.
 * Keeping this in one module means the two CLIs always emit identical structure.
 */
...
const CAMERA_ANGLES = [
  'low-angle hero shot', 'slow push-in (dolly)', 'over-the-shoulder medium',
  'top-down birds-eye', 'tracking side profile', 'close-up on face/detail',
  'wide establishing', 'dutch-angle tension shot',
];
const MOTIONS = [
  'slow dolly-in, subtle parallax', 'smooth orbit around subject',
  'handheld micro-shake for energy', 'rapid push then settle', 'crane up reveal',
  'whip-pan transition out', 'slow-motion emphasis', 'static hold with subject motion',
];
const LIGHTING = [
  'dramatic rim light, dark background', 'soft golden-hour key light',
  'cool blue moody backlight', 'high-key bright and clean',
  'neon practical lights, teal/magenta', 'single hard spotlight, deep shadows',
  'overcast soft diffuse light', 'warm firelight glow',
];
...
// Scene assembly
// ---------------------------------------------------------------------------
// `sceneSeeds` (optional) is an array of { beat?, subject, environment, voiceover }
// — used by the trend builder / LLM mode. When absent, deterministic templates fill in.
function buildScenes(idea, sceneSeeds) {
  ...
  return SCENE_ARC.slice(0, count).map((arc, i) => {
    const seed = (sceneSeeds && sceneSeeds[i]) || {};
    const start = cursor;
    cursor += per;
    return {
      n: i + 1,
      beat: seed.beat || arc.beat,
      purpose: arc.purpose,
      start,
      end: cursor,
      duration: per,
      camera: seed.camera || CAMERA_ANGLES[i % CAMERA_ANGLES.length],
      motion: seed.motion || MOTIONS[i % MOTIONS.length],
      lighting: seed.lighting || LIGHTING[i % LIGHTING.length],
      subject: seed.subject || `Primary subject of "${idea}" expressed for the ${arc.beat} beat`,
      environment:
        seed.environment ||
        `Environment that visually communicates: ${arc.purpose.toLowerCase()}`,
      voiceover: seed.voiceover || defaultVoiceLine(idea, arc.beat),
    };
  });
}

function defaultVoiceLine(idea, beat) {
  switch (beat) {
    case 'HOOK': return `Stop scrolling. This is ${idea} — and it's not what you think.`;
    case 'SETUP': return `Here's the thing about ${idea} that almost nobody notices.`;
    case 'ESCALATION': return `But it gets bigger. Watch what happens next.`;
    case 'TURN': return `And that's when everything about ${idea} changes.`;
    case 'PAYOFF': return `That's the part that sticks with you.`;
    case 'BUTTON': return `Follow for more. You'll want to see what's next.`;
    default: return `${idea}.`;
  }
}
...
function conceptMD(ctx) {
  const sourceLine = ctx.source
    ? `- **Trend source:** ${ctx.source.name} (${ctx.source.category}) — ${ctx.source.link}\n`
    : '';
  const riskBlock = ctx.riskNotes
    ? `\n## ⚠️ Risk / review notes\n${ctx.riskNotes}\n`
    : '';
  return `# Concept — ${ctx.title}

- **Idea / angle:** ${ctx.idea}
${ctx.topicLabel ? `- **Topic label:** ${ctx.topicLabel}\n` : ''}- **Date:** ${ctx.date}
- **Slug:** ${ctx.slug}
${sourceLine}- **Format:** ${DEFAULTS.aspectRatio} vertical · ~${DEFAULTS.totalSecondsTarget}s · ${ctx.scenes.length} scenes
- **Tone:** ${DEFAULTS.tone}
- **Platforms:** ${DEFAULTS.platforms.join(', ')}

## Logline
${ctx.logline || `A fast, cinematic short exploring ${ctx.idea}.`}

## Hook (first 1–2 seconds)
> ${ctx.scenes[0].voiceover}

Alternate hooks (test the strongest):
${(ctx.altHooks && ctx.altHooks.length
    ? ctx.altHooks
    : [ctx.scenes[0].voiceover, `Most people get ${ctx.idea} wrong. Here's the truth.`])
    .map((h, i) => `${i + 1}. ${h}`).join('\n')}

## Why it retains
- Opens on a pattern-interrupt, no intro fluff.
- One angle, one arc — no subplots.
...
---
**Editing notes:** cut to the hook in under 1s, keep every line tight, end on the button
beat with the on-brand signature stamp.
`;
}
...
${ctx.scenes.length} scenes · ${DEFAULTS.aspectRatio} · 4–7s each.
...
- **Continuity notes:** start from the locked Scene ${s.n} key frame; preserve subject design + accent color; smooth match into the next scene.
- **Negative prompt:** ${DEFAULTS.negativePrompt}
...
Vertical ${DEFAULTS.aspectRatio}. Animate each locked key frame into a 4–7s clip, then assemble in order.

${blocks}`;
}

function thumbnailMD(ctx) {
  const turn = ctx.scenes[3] ? 4 : 1;
  return `# Thumbnail Concept — ${ctx.title}

- **Primary image:** the most striking frame from Scene ${turn} (the TURN/PAYOFF beat).
- **Composition:** subject large, off-center; eyes/face or key object in the top two-thirds.
...
- **X:** ${ctx.idea} ${ctx.hashtags.slice(0, 2).join(' ')}
...
function captionsSRT(ctx) {
  ...
  for (const s of ctx.scenes) {
    ...
    chunks.forEach((text, ci) => {
      lines.push(`${idx}`);
      lines.push(`${srtTime(s.start + ci * slice)} --> ${srtTime(s.start + (ci + 1) * slice)}`);
  ...
  return JSON.stringify(
    {
      ...
      date: ctx.date,
      idea: ctx.idea,
      topic_label: ctx.topicLabel || null,
      ...
      tone: DEFAULTS.tone,
      style: DEFAULTS.style,
      trend_score: ctx.trendScore != null ? ctx.trendScore : null,
      risk_notes: ctx.riskNotes || null,
      needs_human_review: !!ctx.needsReview,
      scenes: ctx.scenes.map((s) => ({
        n: s.n, beat: s.beat, start: s.start, end: s.end, duration: s.duration,
      ...
      status: 'draft',
      generated_by: ctx.generatedBy || 'Higgsfield_Youtubengine',
    },
    null,
    2
  );
...
// Public API
// ---------------------------------------------------------------------------
function writePackage(outputsDir, ctx) {
...
// Build a generic deterministic ctx from a plain idea string.
function buildGenericContext(idea, opts = {}) {
  const date = todayISO();
  const slug = opts.slug ? slugify(opts.slug) : slugify(idea);
  const title = opts.title || titleCase(idea);
  const scenes = buildScenes(idea, opts.sceneSeeds);
  ...
  const description =
    opts.description ||
    `A fast, cinematic short about ${idea}. Watch to the end for the turn. ` +
      `New shorts regularly — follow so you don't miss the next one.`;
  const hashtags =
    opts.hashtags || ['#shorts', `#${slug.replace(/-/g, '')}`, '#cinematic', '#fyp', '#storytime'];
  return {
    idea, date, slug, title, scenes, titles, description, hashtags,
    topicLabel: opts.topicLabel, logline: opts.logline, altHooks: opts.altHooks,
    source: opts.source || null,
    riskNotes: opts.riskNotes || null,
    needsReview: !!opts.needsReview,
    trendScore: opts.trendScore != null ? opts.trendScore : null,
    slugBase: opts.slugBase,
    generatedBy: opts.generatedBy || 'Higgsfield_Youtubengine/new-video',
  };
}

module.exports = {
  DEFAULTS, SCENE_ARC, CAMERA_ANGLES, MOTIONS, LIGHTING,
  slugify, titleCase, todayISO, srtTime, uniqueDir,
  buildScenes, defaultVoiceLine, buildGenericContext, writePackage,
};
